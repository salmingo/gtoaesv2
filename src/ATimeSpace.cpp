...
void ATimeSpace::SetJD(double jd) {
	int iy, im, id;
	double fd;
	Mjd2Cal(jd - AU_MJD0, iy, im, id, fd);
	SetUTC(iy, im, id, fd);
}
...
double ATimeSpace::UTC2TAI(double mjd) {
	...
	fd *= (AU_DAYSEC + dleap) / AU_DAYSEC;

	/* Scale from (pre-1972) UTC seconds to SI seconds. */
	fd *= (AU_DAYSEC + dlod) / AU_DAYSEC;

	return (ModifiedJulianDay(iy, im, id, fd) + dat0 / AU_DAYSEC);
}

double ATimeSpace::TAI2UT1(double mjd, double dta) {
	return (mjd + dta / AU_DAYSEC);
}
...
double ATimeSpace::GreenwichMeanSiderealTime(double mjd) {
	...
	return (cycmod(gmst, 360.0) * AU_D2R);
}
...
double ATimeSpace::LocalMeanSiderealTime(double mjd, double lgt) {
	return cycmod(GreenwichMeanSiderealTime(mjd) + lgt, AU_2PI);
}
...
void ATimeSpace::Nutation(double t, double& nl, double& no) {
	...
	static const double U2R = AU_AS2R * 1e-7;
	/* ---------------------------------------- */
	/* Fixed offsets in lieu of planetary terms */
	/* ---------------------------------------- */
	static const double DPPLAN = -0.135E-6 * AU_AS2R;
	static const double DEPLAN =  0.388E-6 * AU_AS2R;
...
void ATimeSpace::SunPosition(double t, double& ra, double& dec) {
	double M = cycmod(125.04 - 1934.136 * t, 360.0) * AU_D2R;
	double l = TrueLongSun(t) - (5.69E-3 + 4.78E-3 * sin(M)) * AU_D2R;
	double eps = MeanObliquity(t) + 2.56E-3 * cos(M) * AU_D2R;
...
double ATimeSpace::Epoch() {
	return (2000.0 + (ModifiedJulianDay() - AU_MJD2K) / 365.25);
}

double ATimeSpace::JulianDay() {
	if (!valid_[ATS_JD]) {
		values_[ATS_JD] = ModifiedJulianDay() + AU_MJD0;
		valid_[ATS_JD]  = true;
	}
...
double ATimeSpace::TAI() {
	if (!valid_[ATS_TAI]) {
		values_[ATS_TAI] = ModifiedJulianDay() + DeltaAT() / AU_DAYSEC;
		valid_[ATS_TAI]  = true;
	}
...
double ATimeSpace::GreenwichMeanSiderealTime() {
	if (!valid_[ATS_GMST]) {
		...
		double gmst = 280.46061837 + t * (13185000.77005374225 + t * (3.87933E-4 - t / 38710000.0));
		values_[ATS_GMST] = cycmod(gmst, 360.0) * AU_D2R;
...
double ATimeSpace::LocalMeanSiderealTime() {
	if (!valid_[ATS_LMST]) {
		values_[ATS_LMST] = cycmod(GreenwichMeanSiderealTime() + lgt_, AU_2PI);
...
double ATimeSpace::LocalSiderealTime() {
	if (!valid_[ATS_LST]) {
		values_[ATS_LST] = cycmod(GreenwichSiderealTime() + lgt_, AU_2PI);
		valid_[ATS_LST]  = true;
	}
...
double ATimeSpace::CenterSun() {
	if (!valid_[ATS_CENTER_SUN]) {
		...
		double c = (1.914602 - (4.817E-3 + 1.4E-5 * t) * t) * sin(ma)
				+ (0.019993 - 1.01E-4 * t) * sin(2 * ma)
				+ 2.89E-4 * sin(3 * ma);
		values_[ATS_CENTER_SUN] = c * AU_D2R;
...
void ATimeSpace::SunPosition(double& ra, double& dec) {
	if (!valid_[ATS_POSITION_SUN]) {
		SunPosition(JulianCentury(), ra, dec);

		values_[ATS_POSITION_SUN_RA]  = ra;
		values_[ATS_POSITION_SUN_DEC] = dec;
...
void ATimeSpace::MoonPosition(double mjd, double&r, double& ra, double& dec) {
	AMath math;
	double mjd0 = mjd - 51543; // 当日零时
	// 月亮轨道根数
	double N = (125.1228 - 0.0529538083 * mjd0) * AU_D2R;
	double i = 5.1454 * AU_D2R;	// 倾度
	double w = (318.0634 + 0.1643573223 * mjd0) * AU_D2R;	// 近地点
	double a = 60.2666;	// 平均距离
	double e = 0.054900;	// 离心率
	double M = (115.3654 + 13.0649929509 * mjd0) * AU_D2R;	// 平近点角
	double eo = TrueObliquity();
	// E0,E1:偏近点角
	double E0 = cycmod(M + e * sin(M) * (1 + e * cos(M)), AU_2PI);
	double E1 = cycmod(E0 - (E0 - e * sin(E0) - M) / (1 - e * cos(E0)), AU_2PI);
	// x,y,r,v:月轨坐标
	double x, y;
	double v;
	// x,y,z-eclip,longitude,latitude,黄道坐标
	double xeclip, yeclip, zeclip;
	double longitude, latitude;
	// x.y,z-equat,赤道坐标
	double xequat, yequat, zequat;
	// 修正参数
	double Ls = MeanLongSun();	// 太阳平黄经
	double Lm = N + w + M;	// 月亮平黄经
	double Ms = MeanAnomalySun();	// 太阳平近点角
	double D = Lm - Ls;	// 月亮平均延伸量
	double F = Lm - N;	// 月亮升交角距

	while(fabs(E0 - E1) > 0.005) {
		E0 = E1;
	...
	longitude = 0;
	latitude = 0;
	math.Cart2Sphere(xeclip, yeclip, zeclip, r, longitude, latitude);
	// 计算扰动
	longitude = longitude * AU_R2D - 1.274 * sin(M - 2.0 * D) + 0.658 * sin(2.0 * D)
		- 0.186 * sin(Ms) - 0.059 * sin(2.0 * (M - D)) - 0.057 * sin(M - 2 * D + Ms)
		+ 0.053 * sin(M + 2.0 * D) + 0.046 * sin(2.0 * D - Ms) + 0.041 * sin(M - Ms)
		- 0.035 * sin(D) - 0.031 * sin(M + Ms) - 0.015 * sin(2.0 * (F - D))
		+ 0.011 * sin(M - 4.0 * D);
	latitude = latitude * AU_R2D - 0.173 * sin(F - 2.0 * D) - 0.055 * sin(M - F - 2.0 * D)
		-0.046 * sin(M + F - 2.0 * D) + 0.033 * sin(F + 2.0 * D)
		+ 0.017 * sin(2.0 * M + F);
	r = r - 0.58 * cos(M - 2.0 * D) - 0.46 * cos(2.0 * D);
	// 转换为赤道坐标
	math.Sphere2Cart(r, longitude * AU_D2R, latitude * AU_D2R, xeclip, yeclip, zeclip);

	xequat = xeclip;
	yequat = yeclip * cos(eo) - zeclip * sin(eo);
	zequat = yeclip * sin(eo) + zeclip * cos(eo);
	math.Cart2Sphere(xequat, yequat, zequat, r, ra, dec);
}

void ATimeSpace::MoonPosition(double& r, double& ra, double& dec) {
	if (!valid_[ATS_POSITION_MOON]) {
		MoonPosition(ModifiedJulianDay(), r, ra, dec);
		values_[ATS_POSITION_MOON_R]   = r;
		values_[ATS_POSITION_MOON_RA]  = ra;
		values_[ATS_POSITION_MOON_DEC] = dec;
		values_[ATS_POSITION_MOON]     = true;
	}
	else {
		r   = values_[ATS_POSITION_MOON_R];
		ra  = values_[ATS_POSITION_MOON_RA];
		dec = values_[ATS_POSITION_MOON_DEC];
	}
}

void ATimeSpace::MoonTopo(double& r, double& ra, double& dec) {
	if (!valid_[ATS_POSITION_MOON_TOPO]) {
		double gclat = (lat_ * AU_R2D - 0.1924 * sin(2.0 * lat_)) * AU_D2R;	// 地心纬度
		double rho = 0.99883 + 0.00167 * cos(2.0 * lat_);		// 地表到地心距离
		double ha;		// 时角
		double g;
		double mpar;	// 视差
		double lmst = LocalMeanSiderealTime();

		MoonPosition(r, ra, dec);
		ha = lmst - ra;
		mpar = asin(1.0 / r);
		g = atan2(tan(gclat), cos(ha));
		ra = ra - mpar * rho * cos(gclat) * sin(ha) / cos(dec);
		if (fabs(g) >= 0.0001)
			dec = dec - mpar * rho * sin(gclat) * sin(g - dec) / sin(g);
		else
			dec = dec + mpar * rho * sin(gclat) * sin(dec) * cos(ha);

		valid_[ATS_POSITION_MOON_TOPO] = true;
		values_[ATS_POSITION_MOON_TOPO_R]   = r;
		values_[ATS_POSITION_MOON_TOPO_RA]  = ra;
		values_[ATS_POSITION_MOON_TOPO_DEC] =dec;
	}
	else {
		r  = values_[ATS_POSITION_MOON_TOPO_R];
		ra = values_[ATS_POSITION_MOON_TOPO_RA];
		dec= values_[ATS_POSITION_MOON_TOPO_DEC];
	}
}
...
double ATimeSpace::JulianCentury(double mjd) {
	return (mjd - AU_MJD2K) / 36525;
}

double ATimeSpace::Epoch(double mjd) {
	return (2000.0 + (mjd - AU_MJD2K) / 365.25);
}
...
double ATimeSpace::TrueRefract(double h0, double airp, double temp) {
	...
	h0 *= AU_R2D;
	R0 = 1.02 / tan((h0 + 10.3 / (h0 + 5.11)) * AU_D2R) + 1.9279E-3;
...
double ATimeSpace::VisualRefract(double h, double airp, double temp) {
	...
	h *= AU_R2D;
	R0 = 1.0 / tan((h + 7.31 / (h + 4.4)) * AU_D2R) + 1.3515E-3;
...
void ATimeSpace::EqTransfer(double rai, double deci, double& rao, double& deco) {
	...
	double eps0= 84381.406 * AU_AS2R;		// J2000对应的黄赤交角
	...
	double K = -20.49552 * AU_AS2R;
	double x, y, z, A, B, C, l, b, dl, db;
	double syml, sx, cx, sb, cb;

	Eq2Eclip(rai, deci, eps0, l, b);
	/* 岁差 */
	x = ((47.0029 - (0.03302 - 6E-5 * t) * t) * t) * AU_AS2R;
	y = (629554.9824 - (869.8089 - 0.03536 * t) * t) * AU_AS2R;
	z = (5029.0966 + (1.11113 - 6E-6 * t) * t) * t * AU_AS2R;
...
void ATimeSpace::EqReTransfer(double rai, double deci, double& rao, double& deco) {
	...
	/* 岁差 */
	x = (47.0029 + (0.03301 + 6E-5 * t) * t) * t * AU_AS2R;
	y = (629554.9824 - (4159.2878 - 1.14649 * t) * t) * AU_AS2R;
	z = (5029.0966 - (1.11113 + 6E-6 * t) * t) * t * AU_AS2R;
	A = (cx = cos(x)) * (cb = cos(b)) * (syml = sin(y - l)) - (sx = sin(x)) * (sb = sin(b));
	B = cb * cos(y - l);
	C = cx * sb + sx * cb * syml;
...
//	h0 = (alt - 0.5667) * AU_D2R; // 减去水平面大气折射, 转换为弧度
	h0 = alt * AU_D2R;
	...
	m0 = cycmod((ra - lgt_ - t0) / AU_2PI, 1.0);

	m = cycmod(m0 - dm / AU_2PI, 1.0);
	t = t0 + 360.985647 * m * AU_D2R;
	SunPosition(JulianCentury(jdn + m), ra, dec);
	Eq2Horizon(t + lgt_ - ra, dec, azi, h);
	m += ((h - h0) / (AU_2PI * cos(dec) * cos(lat_) * sin(t + lgt_ - ra)));
